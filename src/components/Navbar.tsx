          ...
          <div className="hidden md:flex items-center space-x-8">
            {navItems.map((item) => (
              <button
                ...
                className="text-gray-300 hover:text-magical-cyan transition-colors duration-300 relative group"
              >
                {item.name}
                <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-gradient-to-r from-magical-purple to-magical-cyan group-hover:w-full transition-all duration-300"></span>
              </button>
            ))}
          </div>
            ...
            <div className="px-4 py-6 space-y-4">
              {navItems.map((item) => (
                <button
                  key={item.name}
                  onClick={() => scrollToSection(item.href)}
                  className="block w-full text-left text-gray-300 hover:text-magical-cyan transition-colors duration-300 py-2"
                >
                  {item.name}
                </button>
              ))}
